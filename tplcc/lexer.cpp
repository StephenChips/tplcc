...
static const std::vector<std::string> UNSIGNED_INT_SUFFIX{"u", "U"};
...
static const std::vector<std::string> TYPE_FLOAT_SUFFIX{"F", "f"};

static const std::vector<std::string> TYPE_FLOAT_AND_DOUBLE_SUFFIX{"F", "f",
                                                                   "l", "L"};
...
enum class LiteralNumberBase { Octal, Decimal, Hexadecimal };

const std::string* matchesSuffix(
    const std::string& str, size_t offset,
    const std::vector<std::string>& validAlternatives) {
  auto result = std::find_if(validAlternatives.begin(), validAlternatives.end(),
                             [&str, offset](const std::string& suffix) {
...
bool isFirstCharOfExponentPart(const char ch, LiteralNumberBase numberBase) {
  if (numberBase == LiteralNumberBase::Decimal) {
    ...
    // Otherwise it must be a hexadecimal number, since any C octal number can
    // only be an integer.
    return ch == 'p' || ch == 'P';
  }
}
...
class NumberLiteralScanner {
 ...
 private:
  ...
  bool scanSuffixes(
      std::initializer_list<const std::vector<std::string>*> availableSuffixes);
...
bool NumberLiteralScanner::scanSuffixes(
    std::initializer_list<const std::vector<std::string>*> availableSuffixes) {
...
fail:
  ...
  errOut.reportsError({{startOffset, scanner.offset()},
                       "\"" + invalidSuffix +
                           "\" is not a valid suffix for the number literal " +
                           numberLiteralNoSuffix + ".",
                       "invalid suffix."});
  return false;
}
...
std::optional<NumberLiteral>
NumberLiteralScanner::scanAndCreateIntegerLiteral() {
  if (bool hasError = !scanIntegerSuffixes()) return std::nullopt;
  return NumberLiteral{std::move(buffer)};
}

std::optional<NumberLiteral> NumberLiteralScanner::scanAndCreateFloatLiteral(
...
// The number first set of a c-number is {'0' - '9'} and {'.'}. This function
// should only be called when current `is.peek()` is one of the characters in
// the first set.
std::optional<NumberLiteral> NumberLiteralScanner::scan() {
  ...
  // It could be a decimal floating point number, in case it has a fraction
  // part. e.g. 0987.654 is a valid number. We should keep scanning digits until
  // we meet a non-digit, then we can drop a conclusion.
  ...
  //
  // The case (!hasIntegerPart && is.peek() != '.') is impossible. If a number
  // doesn't have an integer part, its first character must be a decimal point
  // '.', otherwise we we call the function even when the current character
  // isn't a digit nor a '.', which is garentee not gonna happen.
  ...
  // We will not meet invalid case like .e10f, .ll, .ace (!hasIntegerPart &&
  // !hasFractionPart)

  const bool hasExponentPart =
      isFirstCharOfExponentPart(scanner.peek(), numberBase);
...
constexpr std::optional<CharSequenceLiteralPrefix> getCharSequencePrefix(
...
void Lexer::scanCharSequenceContent(const char quote, std::string* output) {
  ...
  while (!scanner.reachedEndOfInput() && scanner.peek() != quote &&
         scanner.peek() != '\n') {
...
std::optional<Token> Lexer::scanCharSequence(const char quote,
  ...
  if (auto prefix = getCharSequencePrefix(prefixStr)) {
    ...
    if (quote == '"')
      return StringLiteral{buffer, *prefix};
    else
      return CharacterLiteral{buffer, *prefix};
  } else {
    scanCharSequenceContent(quote, nullptr);
    errOut.reportsError(
        {{startOffset, scanner.offset()},
         quote == '"' ? "\"" + prefixStr +
                            "\" is not a valid prefix for a string literal."
                      : "\"" + prefixStr +
                            "\" is not a valid prefix for a character literal.",
         "Invalid prefix."});
    return std::nullopt;
  }
}
...
std::optional<Token> Lexer::next() {
  ...
  if (scanner.peekN(2) == "/*") {
    scanner.ignoreN(2);
    while (!scanner.reachedEndOfInput() && scanner.peekN(2) != "*/")
      scanner.ignore();
    scanner.ignoreN(2);
    return next();
  }
  ...
  throw std::exception();  // report errors and abort if there is invalid
                           // character in the source code. e.g. ` and @.
}

std::string Lexer::readIdentString() {
  std::string buffer;

  while (std::isdigit(scanner.peek()) || std::isalpha(scanner.peek()) ||
         scanner.peek() == '_') {
    buffer.push_back(scanner.get());
  }
...
std::optional<Keyword> Lexer::findKeyword(const std::string& str) {
  auto pos = std::lower_bound(
      keywordPairs.begin(), keywordPairs.end(), str,
